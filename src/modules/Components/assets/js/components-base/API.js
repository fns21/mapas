...
globalThis.fetch = async (...args) => {
	...
	const before = new CustomEvent("beforeFetch", {
...
globalThis.useEntitiesLists = Pinia.defineStore("entitiesLists", {
	...
	actions: {
		...
		fetchEntityLists(entity, scope) {
			...
			this[scope].ENTITY_LISTS[objectId] =
				this[scope].ENTITY_LISTS[objectId] || [];
...
class API {
	...
	async findOne(id, select) {
		let url = this.createApiUrl("findOne", {
			id: `EQ(${id})`,
			"@select": select || "*",
		});
	...
	async find(query, list, rawProcessor) {
		...
		rawProcessor =
			typeof rawProcessor == "function" ? rawProcessor : undefined;
		...
		return this.GET(url).then((response) => {
			response.json().then((objs) => {
				...
				result.metadata = JSON.parse(
					response.headers.get("API-Metadata")
				);
